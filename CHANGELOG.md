...
- Published to PyPI as `farsight-recon` via GitHub Actions Trusted Publishing (OIDC, no stored API token); `pip install farsight-recon` / `poetry add farsight-recon`.
- `.github/FUNDING.yml` to enable the GitHub Sponsors button on the repo.
...
- HTTP `User-Agent` sent by the tool advertised a stale version and the old lowercase repo URL; now derived from `__version__` and points at the correct-cased `github.com/seedon198/Farsight`.
