...
class SubdomainEnumerator:
    ...
    async def enumerate(
        ...
        if "crt" in techniques:
            ...
            logger.info(
                f"Found {len(self.discovered)} domains via certificate transparency logs"
            )
        ...
        logger.info(
            f"Total discovered subdomains before validation: {len(self.discovered)}"
        )
            ...
            async with self.session.get(url) as response:
                if response.status == 200:
                    try:
                        ...
                        logger.info(
                            f"Found {len(self.discovered)} subdomains via crt.sh"
                        )
                    except Exception as e:
    ...
    async def _brute_force(self, domain: str) -> None:
        ...
        try:
            ...
            for i in range(0, len(subdomains_to_check), batch_size):
                ...
                logger.debug(
                    f"Brute force progress: {total_checked}/{len(subdomains_to_check)}"
                )
...
async def discover_subdomains(
    domain: str, techniques: Optional[List[str]] = None, max_results: int = 500
) -> List[str]:
    """
