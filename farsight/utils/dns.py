...
class PortScanner:
    ...
    def __init__(
        ...
        self.max_concurrent_scans = max_concurrent_scans or get_config(
    ...
    async def scan_port(self, target: str, port: int) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if banner_data:
                    result["banner"] = banner_data.decode(
                        "utf-8", errors="ignore"
                    ).strip()
...
async def enum_subdomains(
    domain: str, wordlist: Optional[List[str]] = None, concurrency: Optional[int] = None
) -> List[str]:
    """
