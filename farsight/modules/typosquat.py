...
if dnstwist_spec is not None:
    ...
    logger.warning(
        "dnstwist library not installed. Typosquatting detection will be limited."
    )
...
if rapidfuzz_spec is not None:
    ...
    if fuzzywuzzy_spec is not None:
        ...
        logger.warning(
            "No fuzzy matching library available. Similarity scoring will be limited."
        )
...
class TyposquatDetector:
    ...
    async def detect(
        self, domain: str, depth: int = 1, check_dns: bool = True
    ) -> Dict[str, Any]:
        """
    ...
    def _generate_typos(self, domain: str, depth: int) -> List[str]:
        ...
        if DNSTWIST_AVAILABLE:
            ...
            for i in range(len(base_name) - 1):
                new_name = (
                    base_name[:i] + base_name[i + 1] + base_name[i] + base_name[i + 2 :]
                )
                typo_domains.append(f"{new_name}.{tld}")
    ...
    async def _check_domains_active(self, domains: List[str]) -> List[str]:
        ...
        if len(active_domains) < 5 and len(domains) > 20:
            logger.info(
                f"Found only {len(active_domains)} active domains through DNS resolution. Trying socket approach."
            )
        ...
        if len(active_domains) < 3 and len(domains) > 50:
            logger.info(
                "Few active domains found, sampling inactive domains for analysis"
            )
            ...
            logger.info(
                f"Added {len(sampled_domains)} sampled inactive domains for analysis"
            )

        return active_domains

    async def _analyze_domains(
        self, original_domain: str, domains: List[str]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for i, domain in enumerate(domains):
            ...
            if risk_score > 70:
                ...
                status = "Suspicious"
                risk_score += (
                    10  # Increase risk score for domains with email capability
                )
                    ...
                    async with self.session.get(
                        ...
                        if response.status in range(200, 400):
                            try:
                                ...
                                result["content_similarity"] = min(
                                    100, hits * 15
                                )  # Scale hits to a 0-100 score
                            except Exception as e:
                                logger.debug(
    ...
    def _determine_typo_type(self, original: str, typo: str) -> str:
        ...
        if len(original_base) == len(typo_base):
            diff_count = sum(
                1 for i in range(len(original_base)) if original_base[i] != typo_base[i]
            )
            ...
            if diff_count == 1:
                ...
                for i in range(len(original_base)):
                    if original_base[i] != typo_base[i]:
                        if (
                            original_base[i] in homoglyphs
                            and typo_base[i] in homoglyphs[original_base[i]]
                        ):
                            return "Homoglyph"
                        else:
            ...
            for i in range(len(original_base) - 1):
                if (
                    original_base[i] == typo_base[i + 1]
                    and original_base[i + 1] == typo_base[i]
                ):
                    swaps += 1
        ...
        for i in range(len(original_base)):
            duplicated = (
                original_base[: i + 1] + original_base[i] + original_base[i + 1 :]
            )
