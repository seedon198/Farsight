...
class ThreatIntel:
    ...
    async def gather_intelligence(
        self, domain: str, emails: Optional[List[str]] = None, depth: int = 1
    ) -> Dict[str, Any]:
        """
            ...
            async with self.session.get(url, headers=headers) as response:
                if response.status == 200:
                    ...
                    if unique_emails:
                        ...
                        logger.info(
                            f"Found {len(unique_emails)} emails from PhoneBook.cz"
                        )
                else:
        ...
        try:
            ...
            known_leaks = [
                ...
                (
                    ...
                    "2018-11-30",
                    "personal_info",
                    ["marriott.com", "starwoodhotels.com"],
                ),
            ]
            ...
            for leak_name, leak_date, leak_type, affected in known_leaks:
                for entity in affected:
                    if domain.lower() == entity or domain.lower().endswith(
                        f".{entity}"
                    ):
                        self.results["dark_web"].append(
                            {
            ...
            if emails:
                for email in emails:
                    ...
                    for leak_name, leak_date, leak_type, affected in known_leaks:
                        for entity in affected:
                            if (
                                email_domain.lower() == entity
                                or email_domain.lower().endswith(f".{entity}")
                            ):
                                self.results["dark_web"].append(
                                    {
                                        "type": leak_type,
    ...
    async def check_haveibeenpwned(self, email: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if is_api_configured("hibp"):
                ...
                # Query for breaches
                headers = {
                    "hibp-api-key": get_config("hibp_api_key", ""),
                    "User-Agent": "Farsight Framework",
                }
                response = await handler.get(
                    f"breachedaccount/{email}", headers=headers
                )
            ...
            logger.warning(
                "HaveIBeenPwned API key not configured. Using alternative check method."
            )
            ...
            for breach_domain, name, date, count in known_breaches:
                if domain.lower() == breach_domain or domain.lower().endswith(
                    f".{breach_domain}"
                ):
